...
class OZResolver {
    ...
    async resolve(transaction) {
        let contractAddress;
        try {
            contractAddress = await this.hre.upgrades.erc1967.getImplementationAddress(transaction.to);
        ...
        try {
            const beaconAddress = await this.hre.upgrades.erc1967.getBeaconAddress(transaction.to);
            contractAddress = await this.hre.upgrades.beacon.getImplementationAddress(beaconAddress);
            const contractName = await this.resolveViaCache(contractAddress);
            if (contractName)
                return contractName;
        }
        ...
        try {
            contractAddress = await this.hre.defender.erc1967.getImplementationAddress(transaction.to);
            const contractName = await this.resolveViaCache(contractAddress);
            if (contractName)
                return contractName;
        }
        ...
        try {
            const beaconAddress = await this.hre.defender.erc1967.getBeaconAddress(transaction.to);
            contractAddress = await this.hre.defender.beacon.getImplementationAddress(beaconAddress);
            const contractName = await this.resolveViaCache(contractAddress);
            if (contractName)
                return contractName;
        }
